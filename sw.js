const CACHE='wakuwaku-bingo-v7';
const ASSETS=['./','./index.html','./style.css','./engine.js','./app.js','./machine.js','./icon.svg','./icon-192.png','./icon-512.png','./manifest.webmanifest'];
self.addEventListener('install',e=>e.waitUntil(caches.open(CACHE).then(c=>c.addAll(ASSETS)).then(()=>self.skipWaiting())));
self.addEventListener('activate',e=>e.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith('wakuwaku-bingo-')&&k!==CACHE).map(k=>caches.delete(k)))).then(()=>self.clients.claim())));
