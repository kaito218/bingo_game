...
function refreshGame(){machine.reset();save();sync();activeMode=state.settings.mode==='random'?'machine':state.settings.mode;paint(0);}
...
$('scene').width=1280;$('scene').height=960;ctx.scale(2,2);
const machine=new BingoMachine();let lastRattle=0;
function rattle(impact){
 if(!state.settings.sound||!audio||audio.state!=='running'||impact<85||performance.now()-lastRattle<65)return;
 lastRattle=performance.now();const t=audio.currentTime,o=audio.createOscillator(),g=audio.createGain();
 o.type='triangle';o.frequency.setValueAtTime(650+Math.random()*1000,t);o.frequency.exponentialRampToValueAtTime(240,t+.025);
 g.gain.setValueAtTime(Math.min(.035,impact/16000),t);g.gain.exponentialRampToValueAtTime(.001,t+.035);
...
function paint(p){ctx.clearRect(0,0,640,480);ctx.save();const shake=busy&&activeMode!=='machine'?Math.sin(p*65)*(p<.65?5:1):0;ctx.translate(shake,0);if(activeMode==='machine'||activeMode==='random'){
 if(busy){machine.advance(p,animDuration);rattle(machine.impact);}machine.draw(ctx,p);
